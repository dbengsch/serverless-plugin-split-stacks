  ...
  setDependsOn(resource, logicalId, normalizedLambdaName) {
    ...
    if (resource.Type === 'AWS::Lambda::Function') {
      ...
      if (dependsOnLambdaName) {
        const provider = this.plugin.serverless.getProvider('aws');
        dependsOnLogicalId = provider.naming.getLambdaLogicalId(dependsOnLambdaName);
      }
    } else {
      if (this.lambdaStacks[normalizedLambdaName]) {
        const lastIndex = this.lambdaStacks[normalizedLambdaName].length - 1;
        dependsOnLogicalId = this.lambdaStacks[normalizedLambdaName][lastIndex];
      } else {
        const provider = this.plugin.serverless.getProvider('aws');
        dependsOnLogicalId = provider.naming.getLambdaLogicalId(normalizedLambdaName);
        this.lambdaStacks[normalizedLambdaName] = []
      }

      this.lambdaStacks[normalizedLambdaName].push(logicalId);
    }
