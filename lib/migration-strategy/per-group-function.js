...
class PerGroupFunction extends PerFunction {
  ...
  constructor(plugin) {
    ...
    if (plugin.config.perGroupFunction) {
      ...
      this.parallelDeployments = 1;
      this.lambdaStacks = {};

      if (
        plugin.config.parallelDeployments
        && typeof plugin.config.parallelDeployments === 'number'
        && Number.isInteger(plugin.config.parallelDeployments)
        && plugin.config.parallelDeployments > 0
      ) {
        this.parallelDeployments = plugin.config.parallelDeployments;
      }
    }
  }
  ...
  setDependsOn(resource, logicalId, normalizedLambdaName) {
    ...
    if (!this.lambdaStacks[nestedStackName]) {
      this.lambdaStacks[nestedStackName] = new Array(this.parallelDeployments)
        .fill([], 0, this.parallelDeployments);
    }
  ...
  getNestedStackName(normalizedLambdaName) {
    ...
    return Number.parseInt(hash.substring(0,4), 16) % this.nestedStackCount;
  }
}
