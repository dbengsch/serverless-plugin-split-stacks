...
module.exports = class PerFunction {
  ...
  constructor(plugin) {
    ...
    if (plugin.config.perFunction) {
      ...
      if (plugin.config.chainCount
        ...
        && Number.trunc(plugin.config.chainCount) > 0
      ) {
        const provider = plugin.serverless.getProvider('aws');
        this.chainCount = plugin.config.chainCount;
        this.logicalIds = {};

        Object.keys(plugin.serverless.service.functions).forEach(lambdaName => {
          const normalizedLambdaName = this.plugin.provider.naming.getNormalizedFunctionName(lambdaName);
          this.logicalIds[normalizedLambdaName] = provider.naming.getLambdaLogicalId(lambdaName);
        });
      }
    }
  }
    ...
    if (this.lambdaNames) {
      ...
      this.lambdaNames.some(normalizedLambdaName => {
        if (logicalId.startsWith(normalizedLambdaName)) {
          lambdaName = this.getLambdaParentName(normalizedLambdaName);
  ...
  getApiGatewayResourceMap(serverless) {
    ...
    apiGatewayPlugin.validated.events.map(({ functionName, http }) => {
      ...
      const normalizedLambdaName = this.getLambdaParentName(
        this.plugin.provider.naming.getNormalizedFunctionName(functionName)
      );
  ...
  getLambdaParentName(lambdaName) {
    if (!this.chainCount) {
      return lambdaName;
    }

    const logicalId = this.logicalIds[lambdaName];
    ...
    const parentLambdaName = index >= this.chainCount ? this.lambdaNames[index - this.chainCount] : null;

    if (parentLambdaName && resource.Type === 'AWS::Lambda::Function') {
      const parentLogicalId = provider.naming.getLambdaLogicalId(parentLambdaName);
      const dependsOn = [parentLogicalId];
